...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('backgrounds.json')
        .then(response => response.json())
        .then(data => {
            defaultBackground = data.find(item => item.name === 'Default');
    ...
    // Add global drag start and end event listeners for cursor management
    document.addEventListener('dragstart', function(event) {
        // Add dragging class to body to show grabbing cursor everywhere
        document.body.classList.add('dragging');
    });

    document.addEventListener('dragend', function(event) {
        // Remove dragging class from body when drag ends
        document.body.classList.remove('dragging');
    });

    // Also handle the case where drag is cancelled (like dragging outside the window)
    document.addEventListener('dragleave', function(event) {
        // Only remove if we're leaving the entire document
        if (event.target === document.documentElement) {
            document.body.classList.remove('dragging');
        }
    });

    // Ensure the class is removed if the drag operation is interrupted
...
function resetAllFilters() {
    ...
    const buttons = document.querySelectorAll('#filterContainer .collection-button, #filterContainer .color-button, #filterContainer .category-button, #filterContainer .footprint-button');
...
function setExclusiveCollection(collection) {
    ...
    activeCollections.clear();
    document.querySelectorAll('#collectionsDropdown .collection-button').forEach(btn => btn.classList.remove('active'));
    
    // Set only the double-clicked collection
    activeCollections.add(collection.toLowerCase());
    const collectionButton = [...document.querySelectorAll('#collectionsDropdown .collection-button')].find(button =>
...
function setExclusiveColor(color) {
    ...
    activeColors.clear();
    document.querySelectorAll('#colorsDropdown .color-button').forEach(btn => btn.classList.remove('active'));
    
    // Set only the double-clicked color
    activeColors.add(color.toLowerCase());
    const colorButton = [...document.querySelectorAll('#colorsDropdown .color-button')].find(button =>
...
function setExclusiveCategory(category) {
    ...
    activeCategories.clear();
    document.querySelectorAll('#categoriesDropdown .category-button').forEach(btn => btn.classList.remove('active'));
    
    // Set only the double-clicked category
    activeCategories.add(category.toLowerCase());
    const categoryButton = [...document.querySelectorAll('#categoriesDropdown .category-button')].find(button =>
...
function setExclusiveFootprint(footprint) {
    ...
    activeFootprints.clear();
    document.querySelectorAll('#footprintDropdown .footprint-button').forEach(btn => {
    ...
    const selectedFootprintButton = [...document.querySelectorAll('#footprintDropdown .footprint-button')].find(button =>
...
function filterItems() {
    const items = document.querySelectorAll('#AllItems .catalog-item'); // Only target Items section
...
function setActiveFootprint(footprint, isActive) {
    const footprintButton = [...document.querySelectorAll('#footprintDropdown .footprint-button')].find(button =>
        button.querySelector('span').textContent.trim().toLowerCase() === footprint.toLowerCase()
    );
...
function drag(event) {
    ...
    // Handle wallpaper/flooring/background dragging
    if (event.target.dataset.source === 'catalog' && 
        (event.target.closest('#AllWallpaperFlooring') || event.target.closest('#AllBackgrounds'))) {
        
        // Use the catalog image as drag image (maintains catalog size)
        /* const dragImage = document.createElement('img');
        dragImage.src = event.target.src; // Use the catalog image source
...
function drop(event) {
    ...
    console.log('Current popupDragData:', window.popupDragData);

    // Handle wallpaper/flooring popup drag - CHECK THIS FIRST AND MOST SPECIFICALLY
    if (data === "popup-wallpaper-flooring-item") {
        console.log('Detected wallpaper/flooring popup drag');
        if (window.popupDragData) {
            console.log('Handling wallpaper/flooring popup drag', window.popupDragData);
    ...
    // Handle backgrounds popup drag
    if (data === "popup-background-item") {
        console.log('Detected background popup drag');
        if (window.popupDragData) {
            console.log('Handling background popup drag', window.popupDragData);
            const dragData = window.popupDragData;
    ...
    // Handle regular popup preview drag (Items) - ONLY if not wallpaper/flooring/background
    if (data === "popup-preview-item") {
        console.log('Detected regular item popup drag');
        if (window.popupDragData) {
            console.log('Handling regular item popup drag', window.popupDragData);
            const dragData = window.popupDragData;
            const dropZone = document.getElementById('dropZone');
    ...
    // Handle catalog drops (wallpaper/flooring/background from catalog - NOT popup)
    const draggedElement = document.getElementById(data);
    if (draggedElement && draggedElement.dataset.source === 'catalog' && 
        (draggedElement.closest('#AllWallpaperFlooring') || draggedElement.closest('#AllBackgrounds'))) {
        
        console.log('Handling catalog drag');
    ...
    // Calculate drop position for regular items
    const dropRect = dropZone.getBoundingClientRect();
    const x = event.clientX - dropRect.left - offsetX;
    const y = event.clientY - dropRect.top - offsetY;

    // Rest of your existing drop function logic for regular catalog items...
    if (!draggedElement) {
        console.log('No dragged element found for ID:', data);
        return;
    }

    console.log('Handling regular catalog item drag');
...
function setWallpaperOrFlooringFromPopup(dragData) {
    const category = dragData.category ? dragData.category.toLowerCase() : null;

    if (category === 'wallpaper') {
        const wallpaperElement = document.getElementById('wallpaper');
        if (wallpaperElement && dragData.dropImage) {
            wallpaperElement.src = dragData.dropImage;
            wallpaperElement.style.left = dragData.fixedLocationLeft || '270px';
            wallpaperElement.style.top = dragData.fixedLocationTop || '205px';
            currentWallpaper = wallpaperElement;
        }
    } else if (category === 'flooring') {
        const flooringElement = document.getElementById('flooring');
        if (flooringElement && dragData.dropImage) {
            flooringElement.src = dragData.dropImage;
            flooringElement.style.left = dragData.fixedLocationLeft || '300px';
            flooringElement.style.top = dragData.fixedLocationTop || '590px';
            currentFlooring = flooringElement;
        }
    }
}

function setBackgroundFromPopup(dragData) {
    const backgroundElement = document.getElementById('background');
    if (backgroundElement && dragData.dropImage) {
        backgroundElement.src = dragData.dropImage;
...
function setWallpaperOrFlooring(catalogImg) {
    console.log('Setting wallpaper/flooring:', catalogImg.alt, catalogImg.dataset);
    
    const category = catalogImg.dataset.category ? catalogImg.dataset.category.toLowerCase() : null;
    console.log('Category:', category);

    if (category === 'wallpaper') {
        const wallpaperElement = document.getElementById('wallpaper');
        if (wallpaperElement && catalogImg.dataset.dropImage) {
            console.log('Setting wallpaper to:', catalogImg.dataset.dropImage);
            wallpaperElement.src = catalogImg.dataset.dropImage;
            wallpaperElement.style.left = catalogImg.dataset.fixedLocationLeft || '270px';
            wallpaperElement.style.top = catalogImg.dataset.fixedLocationTop || '205px';
            currentWallpaper = wallpaperElement;
        } else {
            console.error('Wallpaper element not found or dropImage missing');
        }
    } else if (category === 'flooring') {
        const flooringElement = document.getElementById('flooring');
        if (flooringElement && catalogImg.dataset.dropImage) {
            console.log('Setting flooring to:', catalogImg.dataset.dropImage);
            flooringElement.src = catalogImg.dataset.dropImage;
            flooringElement.style.left = catalogImg.dataset.fixedLocationLeft || '300px';
            flooringElement.style.top = catalogImg.dataset.fixedLocationTop || '590px';
            currentFlooring = flooringElement;
        } else {
            console.error('Flooring element not found or dropImage missing');
        }
    }
}

// New function specifically for backgrounds
function setBackgroundFromCatalog(catalogImg) {
    console.log('Setting background:', catalogImg.alt, catalogImg.dataset);
    
    const backgroundElement = document.getElementById('background');
    if (backgroundElement && catalogImg.dataset.dropImage) {
        console.log('Setting background to:', catalogImg.dataset.dropImage);
        backgroundElement.src = catalogImg.dataset.dropImage;
        backgroundElement.style.left = '0px';
        backgroundElement.style.top = '0px';
        currentBackground = backgroundElement;
    } else {
        console.error('Background element not found or dropImage missing');
    }
}
    ...
    fetch('wallfloor.json')
      ...
      .then(data => {
        data.forEach(item => {
          ...
          img.alt = item.name;
          img.id = item.name.replace(/\s+/g, '-'); // Create unique ID
          img.draggable = true;
          // FIX: Set the correct dropImage path and other needed dataset properties
          ...
          itemContainer.appendChild(img);
          itemContainer.appendChild(label);
          itemContainer.addEventListener('dblclick', () => openWallpaperFlooringPopup(img));
...
function loadCollectionsWallpaperFlooring(collections) {
    ...
    const order = ["Solid", "Pattern", "Wooden", "Tiled", "Stone", "Garden & Patio", "Sports", "Places", "Nature", "Water", "Floral", "Winter", "Spring", "Summer", "Fall", "Valentine", "Halloween", "Christmas"];
    const collectionsArray = Array.from(collections);

    order.forEach(collection => {
        if (collectionsArray.includes(collection.toLowerCase())) {
            // Handle special case for Garden & Patio
            let iconFileName;
            if (collection === "Garden & Patio") {
                iconFileName = "garden&patio.png";
            } else {
                iconFileName = `${collection.toLowerCase()}.png`;
            }
            
            const icon = `assets/icons/wallpaper&flooring/collections/${iconFileName}`;
    ...
    fetch('backgrounds.json')
      ...
      .then(data => {
        data.forEach(item => {
          ...
          img.id = item.name.replace(/\s+/g, '-'); // Create unique ID
          img.draggable = true;
          // FIX: Set the correct dropImage path
          ...
          itemContainer.appendChild(img);
          itemContainer.appendChild(label);
          itemContainer.addEventListener('dblclick', () => openBackgroundsPopup(img));
          container.appendChild(itemContainer);
        });
...
function loadCollectionsBackgrounds(collections) {
    ...
    // Updated order array to include all collections from backgrounds.json
    const order = ["Solid", "Pattern", "Gradient", "Wooden", "Nature", "Floral", "Sky", "Water", "Fall", "Spring", "Summer", "Winter", "Valentines", "Halloween", "Christmas"];
    const collectionsArray = Array.from(collections);
...
function openPopup(item) {
    ...
    if (viewsArray.length > 0) {
        ...
        mainImage.removeEventListener('dragstart', handlePopupDrag);
        mainImage.removeEventListener('dragstart', handleWallpaperFlooringPopupDrag);
        mainImage.removeEventListener('dragstart', handleBackgroundsPopupDrag);
        
        // Add drag event listener for regular items
...
function handlePopupDrag(event) {
    // Don't clear existing data here - set it properly
    ...
    // Get preview image dimensions and calculate scaling
    ...
    // Store all the item data for the drop handler
    ...
    console.log('Regular item drag started:', window.popupDragData); // Debug log

    // Create drag image from current view at natural size
    const views = JSON.parse(event.target.dataset.views);
    const dragImage = document.createElement('img');
    dragImage.src = views[currentViewIndex];
    dragImage.style.opacity = '0.75';
...
function setSmartImageSize(imgElement, containerElement) {
    ...
    imgElement.onload = function() {
        ...
        // Check if this is a wallpaper/flooring or background image (much larger)
        const isWallpaperFlooring = this.dataset.source === 'popup-wallpaper-flooring';
        const isBackground = this.dataset.source === 'popup-background';
        
        let maxScale;
        if (isWallpaperFlooring || isBackground) {
            // For wallpaper/flooring/background images, use much smaller max scale
            maxScale = 0.5; // Much smaller than the 3x for regular items
        } else {
            // Regular items can go up to 3x native size
            maxScale = 3;
        }
        ...
        // Use the smaller of: maxScale OR scale to fit container
        const finalScale = Math.min(maxScale, scaleToFit);
        
        // Ensure minimum scale (but allow very small scales for large images)
        const minScale = (isWallpaperFlooring || isBackground) ? 0.1 : 1;
        const appliedScale = Math.max(minScale, finalScale);
...
function closePopup() {
    ...
    // Clean up event listeners from main image
    const mainImage = document.getElementById('popupMainImage');
    mainImage.removeEventListener('dragstart', handlePopupDrag);
    mainImage.removeEventListener('dragstart', handleWallpaperFlooringPopupDrag);
    mainImage.removeEventListener('dragstart', handleBackgroundsPopupDrag);
    
    // DON'T clear popup drag data here - let the drop handler or timeout do it
    // window.popupDragData = null;
    
    // Reset hidden sections for next popup
    const footprintSection = document.getElementById('popupFootprint').parentElement;
    const categorySection = document.getElementById('popupCategory').parentElement;
    const viewsSection = document.querySelector('.popup-views');
    
    footprintSection.style.display = 'flex';
...
function openWallpaperFlooringPopup(item) {
    const overlay = document.getElementById('popupOverlay');
    const itemName = document.getElementById('popupItemName');
    const category = document.getElementById('popupCategory');
    const collection = document.getElementById('popupCollection');
    ...
    // Set category with icon
    const categoryValue = item.dataset.category || 'N/A';
    const categoryIcon = `assets/icons/wallpaper&flooring/category/${categoryValue.toLowerCase()}.png`;
    category.innerHTML = `<div class="single-item"><img src="${categoryIcon}" alt="${categoryValue}" class="info-icon">${categoryValue}</div>`;

    // Set collection with icon - Fix the data parsing and handle multiple collections
    const itemContainer = item.closest('.catalog-item');
    let collectionsArray = [];
    if (itemContainer && itemContainer.dataset.collection) {
        try {
            collectionsArray = JSON.parse(itemContainer.dataset.collection);
        } catch (e) {
            collectionsArray = [itemContainer.dataset.collection];
        }
    }

    if (collectionsArray.length > 0 && collectionsArray[0] !== '') {
        let collectionItemsHTML = '';
        collectionsArray.forEach(collectionStr => {
            // Proper capitalization for each word
            const normalizedCollection = collectionStr
                .split(/[\s,]+/)
                .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
                .join(' ');
            const collectionIcon = `assets/icons/wallpaper&flooring/collections/${collectionStr.toLowerCase()}.png`;
            collectionItemsHTML += `<div class="collection-item"><img src="${collectionIcon}" alt="${normalizedCollection}" class="info-icon">${normalizedCollection}</div>`;
        });
        collection.innerHTML = collectionItemsHTML;
    } else {
        collection.innerHTML = `<div class="single-item">N/A</div>`;
    }

    // Set colors with stacked layout - Fix the data parsing
    let colorsArray = [];
    if (itemContainer && itemContainer.dataset.colors) {
        try {
            colorsArray = JSON.parse(itemContainer.dataset.colors);
        } catch (e) {
            colorsArray = [itemContainer.dataset.colors];
        }
    }
    ...
    if (colorsArray.length > 0) {
        let colorItemsHTML = '';
        colorsArray.forEach(color => {
            const colorIcon = `assets/icons/wallpaper&flooring/colors/${color.toLowerCase()}.png`;
            const capitalizedColor = color.charAt(0).toUpperCase() + color.slice(1).toLowerCase();
            colorItemsHTML += `<div class="color-item"><img src="${colorIcon}" alt="${capitalizedColor}" class="filter-icon color-icon-small">${capitalizedColor}</div>`;
        });
        colors.innerHTML = colorItemsHTML;
    } else {
        colors.innerHTML = `<div class="single-item">N/A</div>`;
    ...
    // Hide footprint section for wallpaper/flooring
    footprint.parentElement.style.display = 'none';
    ...
    // Set main image and make it draggable
    mainImage.src = item.dataset.dropImage || item.src;
    mainImage.alt = item.alt || item.id;
    mainImage.draggable = true;
    mainImage.dataset.dropImage = item.dataset.dropImage;
    mainImage.dataset.fixedLocationLeft = item.dataset.fixedLocationLeft;
    mainImage.dataset.fixedLocationTop = item.dataset.fixedLocationTop;
    mainImage.dataset.source = 'popup-wallpaper-flooring';
    mainImage.dataset.category = item.dataset.category;
    mainImage.dataset.catalogImage = item.src;
    ...
    // Add the correct drag event listener for wallpaper/flooring
    mainImage.addEventListener('dragstart', handleWallpaperFlooringPopupDrag);
    
    // Apply smart sizing
    setSmartImageSize(mainImage, mainImage.closest('.popup-preview'));

    // Hide views section for wallpaper/flooring
...
function openBackgroundsPopup(item) {
    const overlay = document.getElementById('popupOverlay');
    const itemName = document.getElementById('popupItemName');
    const category = document.getElementById('popupCategory');
    const collection = document.getElementById('popupCollection');
    ...
    // Hide category section for backgrounds
    ...
    if (collectionsArray.length > 0 && collectionsArray[0] !== '') {
        ...
        collectionsArray.forEach(collectionStr => {
            ...
            const normalizedCollection = collectionStr
                ...
                .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
                .join(' ');
            const collectionIcon = `assets/icons/backgrounds/collections/${collectionStr.toLowerCase()}.png`;
            collectionItemsHTML += `<div class="collection-item"><img src="${collectionIcon}" alt="${normalizedCollection}" class="info-icon">${normalizedCollection}</div>`;
        });
        collection.innerHTML = collectionItemsHTML;
    } else {
    ...
    if (colorsArray.length > 0) {
        let colorItemsHTML = '';
        colorsArray.forEach(color => {
            const colorIcon = `assets/icons/backgrounds/colors/${color.toLowerCase()}.png`;
            const capitalizedColor = color.charAt(0).toUpperCase() + color.slice(1).toLowerCase();
            colorItemsHTML += `<div class="color-item"><div class="color-icon-small" style="background-image: url('${colorIcon}');"></div>${capitalizedColor}</div>`;
        });
        colors.innerHTML = colorItemsHTML;
    ...
    // Hide footprint section for backgrounds
    footprint.parentElement.style.display = 'none';
    ...
    mainImage.draggable = true;
    mainImage.dataset.dropImage = item.dataset.dropImage;
    mainImage.dataset.source = 'popup-background';
    mainImage.dataset.catalogImage = item.src;
    ...
    // Add the correct drag event listener for backgrounds
    mainImage.addEventListener('dragstart', handleBackgroundsPopupDrag);
    
    // Apply smart sizing
    setSmartImageSize(mainImage, mainImage.closest('.popup-preview'));

    // Hide views section for backgrounds
    viewsSection.style.display = 'none';
...
function handleWallpaperFlooringPopupDrag(event) {
    event.dataTransfer.setData("text/plain", "popup-wallpaper-flooring-item");
    
    const previewImg = event.target;
    
    // Store data for drop handler
    window.popupDragData = {
        dropImage: event.target.dataset.dropImage,
        fixedLocationLeft: event.target.dataset.fixedLocationLeft,
        fixedLocationTop: event.target.dataset.fixedLocationTop,
        category: event.target.dataset.category,
        alt: event.target.alt,
        adjustedOffsetX: event.offsetX,
        adjustedOffsetY: event.offsetY
    };

    console.log('Wallpaper/Flooring drag started:', window.popupDragData);

    // Create drag image using the stored catalog thumbnail
    const dragImage = document.createElement('img');
    
    if (previewImg.dataset.catalogImage) {
        dragImage.src = previewImg.dataset.catalogImage;
        const approximateScale = 0.3;
        const scaledOffsetX = event.offsetX * approximateScale;
        const scaledOffsetY = event.offsetY * approximateScale;
        ...
        event.dataTransfer.setDragImage(dragImage, scaledOffsetX, scaledOffsetY);
    ...
    // Don't clear popupDragData immediately - let the drop handler do it
    setTimeout(() => {
        closePopup();
        if (document.body.contains(dragImage)) {
            document.body.removeChild(dragImage);
        }
        // Clear the data only after a longer delay to ensure drop completes
        setTimeout(() => {
            if (window.popupDragData) {
                console.log('Clearing popupDragData after timeout');
                window.popupDragData = null;
            }
        }, 500);
    }, 100);
...
function handleBackgroundsPopupDrag(event) {
    event.dataTransfer.setData("text/plain", "popup-background-item");
    ...
    console.log('Background drag started:', window.popupDragData);
