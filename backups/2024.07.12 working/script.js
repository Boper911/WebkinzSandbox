...
let collections = new Set();  // Add this line
let colors = new Set();       // Add this line
let categories = new Set();   // Add this line
let footprints = new Set();   // Add this line
...
function loadColors(colors) {
    ...
    sortedColors.forEach(color => {
        if (!colors.map(c => c.toLowerCase()).includes(color.toLowerCase())) return;
        const button = document.createElement('div');
...
function resetAllFiltersWallpaperFlooring() {
    ...
    // Remove the highlight from filter options
    const buttons = document.querySelectorAll('.collection-button, .color-button, .category-button');
    buttons.forEach(button => button.classList.remove('active'));
