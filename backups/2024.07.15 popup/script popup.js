...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Attach drag and drop events to items in the catalog
    document.querySelectorAll('.catalog-item img').forEach(item => {
        item.draggable = true;
...
function createClonedElement(draggedElement, x, y) {
    const views = JSON.parse(draggedElement.dataset.views);
    const currentViewIndex = parseInt(draggedElement.dataset.currentViewIndex) || 0;
    const clonedElement = document.createElement('img');
    clonedElement.src = `images/items/${views[currentViewIndex]}`;
    clonedElement.alt = draggedElement.alt;
    clonedElement.style.position = 'absolute';
    clonedElement.style.left = `${x}px`;
    clonedElement.style.top = `${y}px`;
...
function loadItems(data) {
    ...
    allItemsContainer.innerHTML = ''; // Clear the container first
    ...
    data.forEach(item => {
        ...
        img.alt = item.name;
        img.dataset.category = item.category;
        img.dataset.collection = JSON.stringify(item.collection || []);
        ...
        detailsButton.addEventListener('click', () => openPopup(img));
...
function openPopup(item) {
    ...
    let collectionsArray = JSON.parse(item.dataset.collection);
    ...
    let colorsArray = JSON.parse(item.dataset.colors);
    ...
    let viewsArray = JSON.parse(item.dataset.views);
    ...
    previewImg.src = `images/items/${viewsArray[0]}`;
    ...
    viewsArray.forEach((view, index) => {
        ...
        viewImg.src = `images/items/${view}`;
        viewImg.onclick = () => {
            previewImg.src = `images/items/${view}`;
        };
        viewContainer.appendChild(viewImg);
        viewsContainer.appendChild(viewContainer);
    });

    for (let i = viewsArray.length; i < 8; i++) {
        const viewContainer = document.createElement('div');
        viewsContainer.appendChild(viewContainer);
    }
