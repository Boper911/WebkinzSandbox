...
function drop(event) {
    ...
    const x = event.clientX - dropRect.left - (draggedElement.offsetWidth / 2);
    const y = event.clientY - dropRect.top - (draggedElement.offsetHeight / 2);
...
function filterLeftItems(category) {
    const items = document.querySelectorAll('#leftCatalogContainer .catalog-item');
    console.log(`Filtering items for category: ${category}`);

    items.forEach(item => {
        console.log(`Item: ${item.id}, Category: ${item.dataset.category}`);
...
fetch('catalog.json')
    ...
    .then(data => {
        ...
        const furnitureTab = document.getElementById('Furniture');
        ...
        data.forEach(item => {
            ...
            allTab.appendChild(itemContainer.cloneNode(true));

            if (item.category === 'Furniture') {
                furnitureTab.appendChild(itemContainer.cloneNode(true));
            } else if (item.category === 'Decorations') {
                decorationsTab.appendChild(itemContainer.cloneNode(true));
            }

            console.log(`Added ${img.src} to ${item.category} tab`);
        });
...
fetch('backgrounds.json')
    ...
    .then(data => {
        ...
        data.forEach(item => {
            ...
            console.log(`Added ${img.src} to background catalog`);
