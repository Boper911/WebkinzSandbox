    ...
    fetch('wallfloor.json')
      ...
      .then(data => {
        data.forEach(item => {
          ...
          const itemContainer = document.createElement('div');
          itemContainer.classList.add('catalog-item'); // <- class for CSS
          itemContainer.dataset.category = item.category;
          itemContainer.dataset.collection = (item.collection || []).map(c => c.toLowerCase()).join(',');
          ...
          img.dataset.category = item.category;
          img.ondragstart = drag;
          img.onclick = () => setBackground(img); // Existing handler
        ...
        // Optional: call filterItemsWallpaperFlooring() if you have one
    ...
    fetch('backgrounds.json')
      ...
      .then(data => {
        data.forEach(item => {
          ...
          img.dataset.source = 'catalog';
          img.ondragstart = drag;
          img.onclick = () => setBackground(img); // Existing behavior
        ...
        // Optional: call filterItemsBackgrounds() if you have one
