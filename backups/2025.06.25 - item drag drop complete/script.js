...
// Preload cache for first view images
const preloadedImages = new Map();

function preloadFirstViewImage(itemName, firstViewSrc) {
    if (!preloadedImages.has(itemName)) {
        const img = new Image();
        img.onload = () => {
            preloadedImages.set(itemName, {
                loaded: true,
        ...
        img.src = firstViewSrc;
        // Mark as loading
        preloadedImages.set(itemName, { loaded: false, src: firstViewSrc });
    }
}
...
function loadItems(data) {
    ...
    data.forEach(item => {
        ...
        // Preload the first view image for smooth dragging
        const firstViewSrc = 'images/items/' + item.views[0];
        preloadFirstViewImage(item.name, firstViewSrc);
...
function drag(event) {
    ...
    // For catalog items, always use the first view (in-room view) for drag image
    // For items already in the room, use their current view
    const dragImageIndex = event.target.dataset.source === 'catalog' ? 0 : currentViewIndex;
    
    // Create and immediately append a drag image
    const dragImage = document.createElement('img');
    dragImage.src = views[dragImageIndex];
    ...
    let adjustedOffsetX = event.offsetX;
    let adjustedOffsetY = event.offsetY;
    
    if (event.target.dataset.source === 'catalog') {
        // Check if we have preloaded image data
        const itemName = event.target.alt || event.target.id;
        const preloadData = preloadedImages.get(itemName);
        
        if (preloadData && preloadData.loaded) {
            // Use actual dimensions from preloaded image
            const maxSize = 200; // Maximum drag image size
            const aspectRatio = preloadData.width / preloadData.height;
            
            let dragWidth, dragHeight;
            if (preloadData.width > preloadData.height) {
                dragWidth = Math.min(maxSize, preloadData.width);
                dragHeight = dragWidth / aspectRatio;
            } else {
                dragHeight = Math.min(maxSize, preloadData.height);
            ...
            const scaleX = dragWidth / event.target.offsetWidth;
            const scaleY = dragHeight / event.target.offsetHeight;
            adjustedOffsetX = event.offsetX * scaleX;
            adjustedOffsetY = event.offsetY * scaleY;
        } else {
            // Fallback to fixed size if preload data not available
            dragImage.style.width = '150px';
            dragImage.style.height = '150px';
            dragImage.style.objectFit = 'contain';
            
            const scaleX = 150 / event.target.offsetWidth;
            const scaleY = 150 / event.target.offsetHeight;
            adjustedOffsetX = event.offsetX * scaleX;
            adjustedOffsetY = event.offsetY * scaleY;
        }
    } else {
        // For room items, maintain their current size
        dragImage.style.width = event.target.style.width || event.target.offsetWidth + 'px';
        dragImage.style.height = event.target.style.height || event.target.offsetHeight + 'px';
    }
    
    document.body.appendChild(dragImage);
    
    // Set the drag image immediately
    event.dataTransfer.setDragImage(dragImage, adjustedOffsetX, adjustedOffsetY);
    
    // Store offsets for drop
