...
function searchItemsLeft() {
    ...
    items.forEach(item => {
        ...
        if (itemName.includes(searchTerm) && 
            (filterCategory === 'all items' || itemCategory === filterCategory) &&
            (activeCollection === 'All' || itemCollection === activeCollection.toLowerCase())) {
            item.style.display = 'block';
        } else {
            item.style.display = 'none';
        }
...
function setActiveCollection(collection) {
    ...
    document.getElementById('collectionsButton').textContent = `Collections: ${collection}`;

    // Hide the popup
    toggleCollectionsPopup();

    filterItems(document.querySelector('.filter-button.active') ? document.querySelector('.filter-button.active').textContent : 'All');
}
...
function loadCollections(collections) {
    ...
    // Sort collections alphabetically and add "All" as the first collection
    const sortedCollections = Array.from(collections).sort();
    sortedCollections.unshift('All');
...
fetch('catalog.json')
    ...
    .then(data => {
        ...
        data.forEach(item => {
            ...
            itemContainer.dataset.collection = item.collection || 'All';

            collections.add(item.collection || 'All');
