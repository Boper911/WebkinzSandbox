...
function setActiveCollection(collection) {
    ...
    // Update the active state of the collection buttons
    const collectionButtons = document.querySelectorAll('.collection-button');
    collectionButtons.forEach(button => {
        button.classList.remove('active');
        if (button.textContent === collection) {
            button.classList.add('active');
        }
    });
...
function loadCollections(collections) {
    ...
    allButton.onclick = () => setActiveCollection('All');

    collectionButtonsContainer.appendChild(allButton);
}
